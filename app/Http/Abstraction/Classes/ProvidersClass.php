...
class ProvidersClass
{
    ...
    {
        $this->providers = array_keys(config('providers.providers'));
    }

    private function valid(string $provider = null): bool
    {
        return in_array($provider, array_keys(config('providers.providers')));
    }
    ...
    {
        return $this->current ?
            config('providers.providers.' . $this->current . '.class') : null;
    }

    public function getFilePath(): string
    {
        return $this->current ?
            env('PROVIDERS_PATH', 'files/')
            . config('providers.providers.' . $this->current . '.file_name')
            . '.json'
            : '';
    }
